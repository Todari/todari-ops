...
코딩 세션은 요청을 규칙으로 분류해 요약·문구 작업은 Sonnet/low, 일반 구현은
Sonnet/medium, 인증·결제·장애·복잡한 설계는 Fable 5.1/high로 시작한다.
SDK 0.3.268에 포함된 CLI 2.1.268과 명시적 모델 ID `claude-fable-5-1`을 사용한다.
`CLAUDE_MODEL` 또는 호출자가 지정한 모델·effort는 우선하며, Haiku에는 effort를 전송하지 않는다.
요청 첫 줄에 `model=sonnet effort=high`처럼 하나 또는 둘 다 직접 지정할 수 있다.
이 선택은 환경변수보다 우선하고 세션 JSON에 저장되어 후속 요청에도 유지된다.

같은 테스트·가설의 반복 실패를 후속 요청에 명시하면 effort를 한 단계 올리며 최대 xhigh로 제한한다.
네트워크·권한 오류는 승격하지 않으며 SDK 예외를 자동 재시도하지 않는다.
일반 질문은 medium, 종료 요약은 low, 장애 진단은 high를 사용한다.
별도 분류 LLM 없이 규칙으로 판단하며 검증·도구 권한은 그대로 적용한다.
`turn:profile`은 선택 이유를, `turn:complete`는 SDK 결과·사용 토큰·비용을 기록한다.
절감률은 아직 측정하지 않았다. 개인 Claude/Codex 앱의 모델 설정은 변경하지 않는다.
